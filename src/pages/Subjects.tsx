...
import "./Subjects.css";
import GameBoardItem, { propsGameBoardItem } from "../components/GameBoardItem";

const SubjectArray = ["Maths", "Computers", "Physics", "Karate"];

const Subjects = () => {
  const [board, setBoard] = useState<propsGameBoardItem[]>([]);
  const [gameStarted, setGameStarted] = useState(false);
  const [activeOpenedItems, setActiveOpeningItems] = useState<number[]>([]);
  const [clicks, setClicks] = useState(0);

  const getNewBoard = () => {
    const l = [...SubjectArray, ...SubjectArray];
  ...
  const onBoardItemClick = (id: number) => {
  ...
  useEffect(() => {
    if (activeOpenedItems.length === 2) {
      const fn = () => {
        const item1: propsGameBoardItem | undefined = board.find(
          ({ id }) => id === activeOpenedItems[0]
        );
        const item2: propsGameBoardItem | undefined = board.find(
          ({ id }) => id === activeOpenedItems[1]
        );
